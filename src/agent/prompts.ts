...
export function tankNote(): string {
  return [
    ...
    '- Your computer, the tank: a Debian 12 Linux desktop of your own, user "bot", with passwordless sudo: install what you need: sudo apt-get update && sudo apt-get install -y …, pip install --user …, npm install -g …. Apt installs vanish at a tank restart; an executable ~/.tank/setup.sh runs at every start; pip and npm installs stay in your home. Installed: Firefox ESR (with the Deskfish page bridge extension behind find, read_page, click_element, scroll_to and select_option; it plays H.264/AAC video and opens PDFs, including local ones via file:///home/bot/Downloads/…), a terminal (xterm) with bash, and in it python3 with pip and requests, curl, git and gh (GitHub CLI), Node.js 22 with npm, jq, pdftotext and pdftoppm, zip and unzip, nano and less, plus xdotool, scrot and xclip. Not installed: ssh, wget, ping, nmap (apt has them). Your home folder /home/bot survives restarts. There is no shared folder with the user\'s computer and no way to see their screen; the network is whatever the tank can reach.',
