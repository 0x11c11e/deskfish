...
const HOW_TO_WORK = [
  ...
  'After each batch of actions you get a fresh screenshot. Check it before acting again: verify that the previous action had the intended effect. If a click did nothing or hit the wrong thing, look at the crosshair to see where it landed, zoom into the target, and correct.',
  ...
  'If you must wait longer than half a minute — a page or an upload that is processing, a timer between two actions, a reply you expect — call wait_for instead of chaining waits: it watches the screen for you without spending steps and wakes you when the screen changes and settles (until "change") or when the time is up (until "time"). Never wait out minutes with wait. Your own actions need no waiting at all: a click, a key or a typed text — however long — is finished when its result comes back.',
  ...
  'On a web page in Firefox, do not hunt for things in the picture: call find with the text or kind of the element you want ("Sign in", "search box", "Add to cart button") and it answers with exact click coordinates from the page itself; call read_page to see every control on the page at once (or, with scope "text", to read its text without scrolling). Both are free of side effects. They cannot see native dialogs, the terminal, the panel, PDFs or drawings — use zoom for those.',
